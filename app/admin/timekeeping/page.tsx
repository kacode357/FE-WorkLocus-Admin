...
// --- THAY ĐỔI 1: Import thêm AlertDialog ---
import {
...
// ... (các interface và hook useDebounce không đổi)
...
// --- THAY ĐỔI 2: Sửa lại SessionCell ---
function SessionCell({ 
    ...
    // Đổi tên hàm và sửa lại để dùng onClick
    const renderReasonButton = (reason?: string | null) => {
        if (!reason) return null;
        return (
             <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onViewReason(reason)}>
    ...
    return (
        <div className="flex flex-col gap-2 text-xs">
            <div className="flex items-center gap-1.5">
                ...
                {renderReasonButton(session.check_in_reason)}
            ...
            <div className="flex items-center gap-1.5">
                ...
                {renderReasonButton(session.check_out_reason)}
                {renderLocationButton(session.check_out_latitude, session.check_out_longitude)}
            </div>
            <div className="flex items-center gap-1.5">
...
export default function TimekeepingPage() {
    ...
    // --- THAY ĐỔI 3: Thêm state cho lý do ---
    ...
    return (
        <>
            <Card>
                {/* CardHeader and CardContent's Filter UI... no changes */}
                <CardHeader>
                                ...
                                {isLoading ? (
                                    ...
                                    attendances.map(item => (
                                        <TableRow key={item._id}>
                                            ...
                                            <TableCell><SessionCell session={item.morning} onViewMap={setViewingLocation} onViewReason={setViewingReason} /></TableCell>
                                            <TableCell><SessionCell session={item.afternoon} onViewMap={setViewingLocation} onViewReason={setViewingReason} /></TableCell>
                                            <TableCell className="font-bold text-base">{item.total_work_time || "N/A"}</TableCell>
                                        </TableRow>
                                    ))
                                ) : (
                                    <TableRow><TableCell colSpan={5} className="h-24 text-center">Không có dữ liệu chấm công.</TableCell></TableRow>
                                )}
                            </TableBody>
                        </Table>
                    </div>
                </CardContent>
                {/* CardFooter for Pagination... no changes */}
                <CardFooter>
                     {pagination && pagination.totalPages > 1 && (
                        <Pagination>
                            <PaginationContent>
                                ...
                                <PaginationItem><span className="px-4 py-2 text-sm">Trang {pagination.currentPage}/{pagination.totalPages}</span></PaginationItem>
                                <PaginationItem>
                                     <PaginationNext href="#" onClick={(e) => { e.preventDefault(); if(currentPage < pagination.totalPages) setCurrentPage(currentPage + 1); }} className={currentPage === pagination.totalPages ? "pointer-events-none opacity-50" : ""}/>
                                </PaginationItem>
                            </PaginationContent>
                        </Pagination>
                       )}
            ...
            {/* --- THAY ĐỔI 4: Thêm AlertDialog để hiển thị lý do --- */}
            <AlertDialog open={!!viewingReason} onOpenChange={() => setViewingReason(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Lý do Check-in/out</AlertDialogTitle>
